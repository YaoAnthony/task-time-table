...
import { InventoryItem } from '../../../../Types/Profile';
...
const Backpack = () => {
    ...
    // Padding grid to make it look like a real RPG inventory (e.g., minimum 30 slots)
    const totalSlots = Math.max(30, Math.ceil(filteredInventory.length / 6) * 6);
    const slots = Array.from({ length: totalSlots });

    const handleUseItem = (item: InventoryItem) => {
        // Dispatch API call or action here in the future
        message.success(`使用了: ${item.name}`);
        // If it was a real app, dispatch(useItemAction(item.inventoryKey))
    };
    ...
    return (
        <section className="w-full h-[85vh] flex flex-col rounded-3xl border border-white/60 dark:border-white/10 
            ...
            <div className="flex flex-1 overflow-hidden relative">
                {/* Background Ambient Layers inside Main Content */}
                                        ...
                                        className={`relative aspect-square rounded-2xl flex items-center justify-center transition-all duration-300
                                            ${item 
                                                ? `bg-white/60 dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 cursor-pointer 
                                                   ...
                                                   (isSelected ? 'ring-2 ring-offset-2 ring-offset-slate-50 dark:ring-offset-neutral-900 ring-blue-500 dark:ring-[#FFC72C] shadow-[0_0_20px_rgba(59,130,246,0.4)] dark:shadow-[0_0_20px_rgba(255,199,44,0.4)]' : 'hover:border-blue-300 dark:hover:border-white/50')
                                    ...
                                    >
                                        {item && (
                                            <>
                                                ...
                                                {/* Rarity/Glow outline simulation based on selection */}
                                                {isSelected && (
                                                    <motion.div 
                                                        layoutId="backpackOutline"
                                                        className="absolute inset-0 rounded-2xl border-[3px] border-blue-500/50 dark:border-[#FFC72C]/50 z-0 pointer-events-none"
                                                        initial={false}
                ...
                <div className="w-[340px] bg-white/30 dark:bg-black/20 flex-col relative shrink-0 hidden lg:flex z-10 border-l border-white/40 dark:border-white/5 backdrop-blur-md">
                    <AnimatePresence mode="wait">
                        {selectedItem ? (
                            ...
                            >
                                {/* Item Details Ambient Background */}
                                ...
                                >
                                    <motion.div 
                                        className="text-[100px] filter drop-shadow-[0_10px_20px_rgba(0,0,0,0.15)] dark:drop-shadow-[0_0_25px_rgba(255,255,255,0.2)]"
                                ...
                                <div className="flex-1 flex flex-col z-10">
                                    <h2 className="text-3xl font-black text-blue-700 dark:text-[#FFC72C] tracking-wider mb-3 drop-shadow-sm">{selectedItem.name}</h2>
                                    <div className="flex justify-between items-center mb-6 text-xs font-bold uppercase tracking-widest text-neutral-500 dark:text-white/50 border-b border-black/5 dark:border-white/10 pb-4">
                                        <span className="bg-white/50 dark:bg-black/30 px-3 py-1.5 rounded-lg border border-black/5 dark:border-white/5">类别: {inventoryTabs.find(t => t.id === selectedItem.type)?.label || selectedItem.type}</span>
                                        <span className="bg-white/50 dark:bg-black/30 px-3 py-1.5 rounded-lg border border-black/5 dark:border-white/5">拥有: {selectedItem.quantity}</span>
                                    </div>
                                    <div className="bg-white/40 dark:bg-black/30 p-5 rounded-2xl border border-white/60 dark:border-white/5 shadow-inner">
                                        <p className="text-neutral-700 dark:text-white/80 leading-relaxed text-sm tracking-wide font-medium">
                                            {/* Fallback description since type doesn't strictly have description */}
                                            {typeof selectedItem.metadata?.description === 'string' 
                                                ? selectedItem.metadata.description 
                                                : "这是一个由虚空物质铸造而成的神秘结构体，其内部流淌着不可预知的能量数据流。"}
                                        </p>
                                    </div>
                                </div>
                                
                                {/* Action Buttons */}
                                <div className="mt-8 pt-6 flex justify-end z-10">
                                    <motion.button 
            ...
            {selectedItem && (
                 ...
                 >
                     <div className="flex items-center gap-4">
                         <div className="text-4xl bg-white/50 dark:bg-black/50 p-2 rounded-xl shadow-inner border border-black/5 dark:border-white/5">{getIconForType(selectedItem.type)}</div>
                         <div>
